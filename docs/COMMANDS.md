...
- ⚠️ **冲突未解决**：git merge/rebase产生的冲突标记
- 🔄 **同步新鲜度**：基础分支是否是最新的（针对start命令）

**🔗 GitHub远程状态（4种，需要gh工具）：**
- 📋 **PR存在性**：是否已创建PR
- 👥 **审核状态**：待审核/已审核/需修改
- ✅ **合并状态**：已合并/待合并/已关闭
- 🔄 **CI状态**：持续集成检查结果
...
**🎯 操作权限状态：**
基于前10种状态的组合，智能确定各个命令的执行权限

| 状态 | gpf start | gpf pr | gpf clean | gpf sync | 说明 |
|------|-----------|---------|-----------|----------|------|
| **未保存修改** | ❌ 阻止 | ❌ 阻止 | ❌ 阻止 | ⚠️ 警告 | 必须先保存 |
| **未提交内容** | ❌ 阻止 | ❌ 阻止 | ❌ 阻止 | ⚠️ 警告 | 必须先提交 |
| **未推送提交** | ✅ 允许 | ❌ 阻止 | ❌ 阻止 | ✅ 允许 | PR需要推送 |
| **冲突未解决** | ❌ 阻止 | ❌ 阻止 | ❌ 阻止 | ❌ 阻止 | 必须先解决冲突 |
| **基础分支过时** | ⚠️ 警告 | ✅ 允许 | ✅ 允许 | ✅ 允许 | start建议先同步 |
| **未创建PR** | ✅ 允许 | ✅ 允许 | ❌ 阻止 | ✅ 允许 | clean需要PR完整 |
| **PR待审核** | ✅ 允许 | ❌ 阻止 | ❌ 阻止 | ⚠️ 警告 | 避免重复PR |
| **PR已合并** | ✅ 允许 | ❌ 阻止 | ✅ 允许 | ✅ 允许 | 可安全清理 |
...
📊 本地Git状态:
  ...
  ✅ 已推送GitHub (0个未推送提交)
  ✅ 无合并冲突
  🔄 分支同步: 基于最新Epic分支 (领先2个提交)

🔗 GitHub远程状态:
  📋 PR #123: OPEN - "Add login functionality"
  👥 审核状态: ⏳ 等待review
  ✅ 可合并状态: 准备就绪
  🔄 CI状态: ✅ 通过
